	...
	private static ByteBuffer makeRGBABuffer(ArrayBitmap image) {
		...
		image.visitAll(new ArrayBitmap.IVisitor() {
			...
			public void visit(int x, int y, int pixel) {
				buffer.put((byte)Color.getARGBComponent(pixel, 1));
				buffer.put((byte)Color.getARGBComponent(pixel, 2));
				buffer.put((byte)Color.getARGBComponent(pixel, 3));
				buffer.put((byte)Color.getARGBComponent(pixel, 0));
			}
		});
		buffer.flip();
		return buffer;
